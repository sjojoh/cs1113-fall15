...
#
def pixellimit (value,quantity):
    if (value + quantity > 255):
       new = 255
...
def intensify(pixel,quantity):
    new0 = pixellimit (pixel[0], quantity)
    new1 = pixellimit (pixel[1],quantity)
    new2 = pixellimit (pixel[2], quantity)
...
def brighten (image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = intensify(pixels3d[x,y],20)
...
def bw(image_surf):    # get pixel dimensions of image
    rows = image_surf.get_size()[0]
    cols = image_surf.get_size()[1]
